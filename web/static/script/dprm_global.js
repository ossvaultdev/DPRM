function WindowsMessage(message) 
{
    alert(message);
}

function replace_value_by_tag(value_to_format)
{
    value_to_format = value_to_format.replaceAll(':','<tag_two_point_h>');
    value_to_format = value_to_format.replaceAll('=','<tag_equal_value>');

    return value_to_format
}

function is_valid_string(value_to_check, valid_value)
{
    for (let i = 0; i < value_to_check.length; i++) 
    {
        let has_value = false
        for (let j = 0; j < valid_value.length; j++) 
        {
